...
const signUpUser = async(req,res)=>{
    ...
    try {
        ...
        if(process.env.NODE_ENV!=='production') res.cookie('token',token,{httpOnly:true,maxAge:60*60*24*7})
        else res.cookie('token',token,{httpOnly:true,maxAge:60*60*24*7,sameSite:'none',secure:true,domain:'https://moments-frontend-netlify.netlify.app/login'})
...
const signOutUser = (req,res)=>{
    if(process.env.NODE_ENV!=='production')res.clearCookie('token',{httpOnly:true,maxAge:60*60*24*7})
    else res.clearCookie('token',{httpOnly:true,maxAge:60*60*24*7,sameSite:'none',secure:true,domain:'https://moments-frontend-netlify.netlify.app/login'})
